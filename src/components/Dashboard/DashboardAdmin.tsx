...
const DashboardAdmin = () => {
  ...
  const renderSelectedShiftInvoiceTimeSheetCard = (item: string) => {
    if (item === "Shifts") {
      return <ShiftsCard isLoading={isLoading} shiftsGraphStats={data?.data?.shiftChart} />;
    } else if (item === "Invoice") {
      return <InvoiceCard isLoading={isLoading} />;
    } else if (item === "TimeSheet") {
      return <TimeSheetCard isLoading={isLoading} timesheetGraphStats={data?.data?.timesheetChart} />;
    }
  };

  const renderRecruitmentStaffClientCard = (item: string) => {
    if (item === "Recruitment") {
      return <ClientCard isLoading={isLoading} recruitmentStats={data?.data?.recruitment} />;
    } else if (item === "Staff") {
      return <StaffCard isLoading={isLoading} staffStats={data?.data?.staff} />;
    } else if (item === "Client") {
      return <RecruitmentCard isLoading={isLoading} clientsStats={data?.data?.client} />;
    }
  };
  ...
  const renderSelectedComprehensionCandidates = (item: string) => {
    if (item === "shiftComprehension") {
      ...
      return <TopCandidates isLoading={isLoading} topCandidatesData={data?.data?.topCandidate} />;
    }
  };
