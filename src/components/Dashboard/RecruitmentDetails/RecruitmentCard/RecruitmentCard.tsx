...
//Main
export const RecruitmentCard = ({ clientsStats, isLoading }: any) => {
  return (
    <Layout className="dashboard bg-unset">
      <Card className="most-used-services-card  border-radius-8 card card-bg-color" bordered={false} style={{ boxShadow: "0px 12px 23px rgba(62, 73, 84, 0.04)", minHeight: "400px" }}>
        ...
        {isLoading ? <div style={{ textAlign: "center", paddingTop: "30px" }}><Spin indicator={<LoadingOutlined style={{ fontSize: 36 }} spin />} /></div> : <ClientGraph clientsStats={clientsStats} isLoading={isLoading} />}
      </Card>
    </Layout>
  );
};
