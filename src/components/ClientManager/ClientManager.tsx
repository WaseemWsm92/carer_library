...
import { useGetRequestQuery } from '../../store/Slices/ClientManager';
...
//query parameters of search and filter
  ...
  const { data, isLoading, isSuccess, isError } = useGetRequestQuery({ query,pagination:pagination,role:"client" })  // get table data
 ...
 
    {
      ...
      render: (_, text) =>
        <div className="cursor-pointer d-flex align-center "  >
          <img src={ isNullOrEmpty(text?.uploadLogo)? `https://ui-avatars.com/api/?rounded=true&name=${text.firstName} ${text.lastName}` :  `https://rnd-s3-public-dev-001.s3.eu-west-2.amazonaws.com/${text?.uploadLogo?.mediaId}.${text?.uploadLogo?.mediaMeta?.extension}`} height={30} width={30} alt="avatar" />
