...
import event from "@antv/g2/lib/chart/event";
...
const ShiftTime = () => {
  ...
  const { data, isLoading, isSuccess, isError } = useGetShifTimeFilterQuery({ refetchOnMountOrArgChange: true });
  const { data: filterData, isLoading: isFilterLoading, isSuccess: isFilterSuccess, isError: isFilterError } = useGetShifTimeQuery({ refetchOnMountOrArgChange: true, selectedFilterValue });
  ...
  const handleShiftTimeSubmit = () => {
    ...
    postShifTime({ payload: newFormattedPayload })

    AppSnackbar({ type: "success", messageHeading: "Successfully Saved!", message: "Information saved successfully" });

  }

  useEffect(() => {

    if (shiftTimeData?.data !== undefined) {
      setTime(
        {
          longDayShiftStart: {
  ...
  }, [shiftTimeData, unchangeUserData])
    ...
    <div className='Shift-Time-Settings'>
      <div className="header bg-white border-radius-8">
        ...
        <Select
          ...
          onChange={(value: string, event: any) => value ? setSelectedFilterValue(value) : setSelectedFilterValue("")}
          value={!selectedFilterValue ? undefined : selectedFilterValue}
          options={optimizedUserRoleDropdown}
          dropdownStyle={{ textTransform: 'capitalize' }}
        />
      </div>
      {(!isFilterLoading && selectedFilterValue) && (
              ...
              <Row>
                ...
                <Col xxl={12} xl={12} lg={12} md={12} xs={24}>
                  <div className="start-time d-flex align-items-center">
                    ...
                    <input type="time" value={time.longDayShiftEnd.longDayShiftEndTime} onChange={(event: any) => setTime({
