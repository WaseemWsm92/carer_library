...
import { useGetRequestQuery, usePostClientsRequestMutation } from '../../../../store/Slices/OnBoarding';



export const { Option } = Select;

const AllocateNewCareHomeModal = ({careCordinatorId}:any) => {
  ...
  const { data, isLoading, isSuccess, isError, } = useGetRequestQuery({ refetchOnMountOrArgChange: true ,pagination,query,role:"client"})
  const [postClientsRequest] =usePostClientsRequestMutation()
  let getClients: any;
  if (isLoading) {
    getClients = <p>Loading...</p>
  }
  else if (isSuccess) {
    getClients = data
  }
  ...
  const dropDownOptions=getClients?.data?.result?.map((clientName:any)=>clientName)
    ...
    const onFinish = (values: any) => {
        ...
        postClientsRequest({payload:{careHomeId:values?.careHomeId,careCoordinatorId:careCordinatorId}})
        setIsModalOpen(false);
    };
