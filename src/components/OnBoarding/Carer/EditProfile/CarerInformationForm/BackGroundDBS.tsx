...
const token: any = JSON.parse(userData);
  ...
  const {data,isLoading,isSuccess,isError}=useGetRequestByIdQuery({  id: state?.editProfile?._id ?? token?.id,detail:"BACKGROUND"})
  let profileViewData:any;
   ...
   if(isSuccess){
    ...
    const [validCandidate, setValidCandidate] = useState<Boolean>(profileViewData?.data?.userprofile[0]?.dbsCheck?.dbsOnline);
    ...
    const carerProfileDataDbs={
      ...
      dbsOnline:profileViewData?.data?.userprofile[0]?. dbsCheck?.dbsOnline ,
      ...
      issueDate:profileViewData?.data?.userprofile[0]?.rightToWork?.issueDate?dayjs(profileViewData?.data?.userprofile[0]?.rightToWork?.issueDate
        ):undefined,
    }

    const onFinish = (values: any) => {
        const payloadDbChecks={...values , certificates: certificateId}
        values && handleSelectedStepValue('Right to Work')
        postBckgroundChecksRequest({payload:{dbsCheck:payloadDbChecks},id:state?.editProfile?._id })
    };
...
>
    <Row gutter={[20, 15]}>
        ...
        {validCandidate  && <Col xs={21}>
            <UploadImage uploadCertificateId={uploadCerticateId} />
        </Col>}
