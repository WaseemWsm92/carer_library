...
const AdditionalDocsFromCandidate = (props: any) => {
    ...
    const {data,isSuccess,isError}=useGetRequestByIdQuery({id:state?.editProfile?._id,detail:"OTHERINFO"})
    ...
    const onFinish = async (values: any) => {
      try {
        if (selectedDropdownOption === "Edit") {
          ...
          const { error }: any = await  postOtherInformationRequest({payload:{additionalDocs:{...values,attachment:certificateId }},id:state?.editProfile?._id})
        ...
        },
        {
            ...
            render: (_, text) =>
                <span onClick={showModal} className='cursor-pointer' >
                    <img src={Edit} alt="edit"  onClick={() =>{ handleAdditionalDocsById(text);setSelectedDropdownOption("Edit")}}/>
                </span>
        },
