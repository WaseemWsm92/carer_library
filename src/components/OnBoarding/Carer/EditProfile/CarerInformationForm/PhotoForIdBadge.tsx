...
const PhotoForIdBadge = (props: any) => {
    ...
    const {data:photpUpload,isSuccess:isSuccessPhoto}=useGetRequestByIdQuery({id:state?.editProfile?._id ??token?.id,detail:"ABOUT"})
    ...
    const [uploadImage, setUploadedImage] = useState<any>({ preview:editProfileImage});
let profileId:any
if(isSuccess){
   profileId=data
}
    ...
    const items: MenuProps["items"] = [
        {
            key: "1",
            label: (
                <Space onClick={() => {setUploadedImage(undefined);  updateRequest({id:state?.editProfile?._id,payload:{profilePhoto:null}})}} size={20}>
    ...
    return (
      ...
      {!(uploadImage) ? (
          <Row gutter={[20, 20]} >
              ...
              <Col xs={24}>
                  <Space>
                      <Button className='edit-module-button   align-center d-flex btn-secondary' onClick={() =>{ updateRequest({id:state?.editProfile?._id,payload:{profilePhoto:profileId?.data?._id}});handleSelectedStepValue('ID Upload (Passport/DL)')}}   >Continue</Button>
                  </Space>
              </Col>
              ...
              <Col xs={24}>
                  <Space className='carer-buttons'>
                   { auditCheck &&  <Tooltip
                      ...
                      <Button className='edit-module-button bg-orange-color  align-center d-flex ' htmlType='submit' onClick={() =>{ updateRequest({id:state?.editProfile?._id,payload:{profilePhoto:profileId?.data?._id}})}} >Save</Button>
