...
const AdditionalTrainingDetails = (props: any) => {
    ...
    const {data,isLoading: isGetLoading,isSuccess}=useGetRequestByIdQuery({id:state?.editProfile?._id,detail:"TRAININGWORK"})
    ...
    const onFinish = async (values: any) => {
      const payloadAdditionalDetails={...values,certificates:certificateId  }
      try {
        if (editAdditionalDetails === "Edit") {
          ...
          const { error }: any = await   postAddAdditionalDetailsRequest({payload:payloadAdditionalDetails,id:state?.editProfile?._id})
    ...
    useEffect(() => {
     ...
     }, [form,payloadAdditionalTrainings])
            ...
            render: (_, text:any) => <span className='fs-14 fw-400 title-color'> <a href={`https://rnd-s3-public-dev-001.s3.eu-west-2.amazonaws.com/${text.certificates?.mediaId}.${text.certificates?.mediaMeta?.extension}`}> <img src={Pdf} alt="image" /></a> </span> },
        {
            ...
            render: (_, text) =>
                <span onClick={showModal} >
                    <img src={Edit} alt="edit" onClick={() => handleReferenceRowData(text)} />
                </span>
                ...
                >
                    <Row gutter={[30, 5]} align="bottom">
                        ...
                        <Col xs={24} sm={24} md={12} lg={12} className='date-picker-wrapper'>
                            ...
                            <Form.Item name="certificateExpiryDate" label="Certificate Expiry Date" style={{ maxWidth: '100%' }}>
                                <DatePicker   popupClassName="date-picker-content" suffixIcon={<img src={DateIcon} />} clearIcon={false} placeholder='dd/mm/yyyy' />
                            </Form.Item>
                        </Col>
