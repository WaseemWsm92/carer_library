...
const UnemployementPeriod = (props: any) => {
  ...
  const [selectedRowData ,setSelectedRowData]=useState<any>({})
    ...
    const {data,isLoading,isSuccess}=useGetWorkExperienceRequestByIdQuery({id:state?.editProfile?._id})
    ...
    const onFinish = async (values: any) => {
      try {
        ...
        if (selectedDropdownOption =="Add") {
          ...
          const { error }: any = await   postUnEmployementRequest({id:state?.editProfile?._id,payload:values})
    ...
    return (
        <div >
            ...
            <Row gutter={[20, 20]}>
                ...
                <Col xs={24}>
                    <Table className='onboading-table-wrapper' columns={UnemployedPeriod} dataSource={unEmployeementTableData?.data?.unemploymentPeriod} scroll={{ x: "max-content" }} pagination={false} />
                </Col>
            ...
            <Modal className='personal-form-wrapper' title={<span style={{ fontWeight: "500px", fontSize: "20px" }}>Add Unemployement Period</span>} centered open={isModalOpen} onOk={handleOk} onCancel={handleCancel} width={800} footer={false} closeIcon={<img src={Close} alt="" />}>
                <Form
                    name="basic"
                ...
                >
                    ...
                    <Row gutter={[30, 5]} align="bottom">
                        ...
                        <Col xs={24} sm={24} md={12} className='date-picker-wrapper'>
                            <Form.Item name="startDate" label="Start Date" {...config} style={{ maxWidth: '100%' }}>
                                <DatePicker popupClassName="date-picker-content" suffixIcon={<img src={DateIcon} />} clearIcon={false} />
                            </Form.Item>

                        </Col>
                        <Col xs={24} sm={24} md={12} className='date-picker-wrapper'>
                            <Form.Item name="endDate" label="End Date" {...config} style={{ maxWidth: '100%' }}>
                                <DatePicker popupClassName="date-picker-content" suffixIcon={<img src={DateIcon} />} clearIcon={false} />
                            </Form.Item>
                        </Col>
