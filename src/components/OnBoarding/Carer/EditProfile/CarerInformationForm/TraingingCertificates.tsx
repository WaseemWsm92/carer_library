...
const TraingingCertificates = (props: any) => {
    ...
    const {data,isSuccess}=useGetRequestByIdQuery({id:state?.editProfile?._id ??token?.id,detail:"TRAININGWORK"})
    ...
    const payloadTrainingCertificates={
      ...
      certificateExpiryDate:trainingCertificates?.data?.userprofile[0]?.tCertificates?.certificateIssuedDate?dayjs(trainingCertificates?.data?.userprofile[0]?.tCertificates?.certificateIssuedDate):undefined
    ...
    const onFinish = (values: any) => {
        ...
        const payloadTrainingCertificate={...trainingCertificateSwitchValues,...values,certificates:certificateId}
        postTrainingCertificatesRequest({payload:payloadTrainingCertificate,id:state?.editProfile?._id,isAudited})
    };
