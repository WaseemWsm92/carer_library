...
import { useDeleteReferenceRequestMutation, useGetReferenceRequestQuery, usePostReferenceRequestMutation, useUpdateReferenceRequestMutation } from '../../../../../store/Slices/References';
...
const Refrences = (props: any) => {
    ...
    const { data,  isSuccess } = useGetReferenceRequestQuery({id:state?.editProfile?._id})
...
const onFinish = async (values: any) => {
  try {
    if (selectedDropdownOption === "Edit") {
      ...
      const { error }: any = await postReferenceRequest({ id: state?.editProfile?._id, payload: { ...values, refRequest: true, approvalStatus: "PENDING" } })
                    ...
                    <Row gutter={[30, 5]} align="bottom">
                        ...
                        <Form.Item name="switch" label="" >
                            <Space className='modal-buttons'>
                                <Button className='modal-button btn-cancel  ' htmlType='submit' >Cancel</Button>
