...
const WorkExperience = (props: any) => {
  ...
  const payloadWorkExperience={
    employerName:selectedRowData?.employerName,
  ...
  const onFinish = async (values: any) => {
    try {
      if (selectedDropdownOption =="Edit") {
        ...
        const { error }: any = await updateWorkExperienceRequest({id:selectedRowData?._id,payload:payload})
      ...
      if (selectedDropdownOption =="Add") {
        const payload={...values,currentlyWorkingHere:true}
        const { error }: any = await   postWorkExperienceRequest({id:state?.editProfile?._id,payload:payload})
      ...
      >
        ...
        >
          <Row gutter={[30, 5]} align="bottom">
            ...
            <Col xs={24}>
              <Form.Item label="Reason of Leaving" name="leavingReason">
                <TextArea rows={4} />
              </Form.Item>
            </Col>
