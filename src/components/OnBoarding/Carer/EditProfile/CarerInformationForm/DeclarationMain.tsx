...
const DeclarationMain = () => {
  ...
  const [declarativeSwitchValues, setDeclarativeSwitchValues] = useState({timeDirectives: profileViewInfoData?.data?.userprofile?.declaration?.timeDirectives,
    challengingBehaviour: profileViewInfoData?.data?.userprofile?.declaration?.challengingBehaviour,
    confidentialAgreement: profileViewInfoData?.data?.userprofile?.declaration?.confidentialAgreement,
    pensionOptout:profileViewInfoData?.data?.userprofile?.declaration?.pensionOptout,
    dataPrivacy:profileViewInfoData?.data?.userprofile?.declaration?.dataPrivacy,
    candidateDeclaration:profileViewInfoData?.data?.userprofile?.declaration?.candidateDeclaration
  });
  const [rightTOworkDetails, setRightTOworkDetails] = useState(profileViewInfoData?.data?.userprofile?.declaration?.rightTOworkDetails);
  ...
  const onFinish = (value: any) => {
    ...
    updateRequest({id:state?.editProfile?._id,payload:{declaration:declarativePayloadValues}})
  }
  return (
    <> {isSuccess ?   <Form
      name="basic"
    ...
    >
      ...
      <Space direction="vertical">
        ...
        <Form.Item name="rightTOworkUK" label={<Space size={3} className='fw-600 fs-14'> <div className="onboarding-small-dots"></div>  <span className="title-color">Does your candidate have right to work in Uk?</span> </Space>} >
          ...
          <Radio.Group defaultValue={profileViewInfoData?.data?.userprofile?.declaration?.rightTOworkUK}>
        ...
        <Form.Item name="dbacheck" label={<Space size={3} className='fw-600 fs-14'> <div className="onboarding-small-dots"></div> <span className="title-color">Are you willing to undergo a full enhanced DBA check at a price of $55 ?</span></Space>} >
          <Radio.Group  defaultValue={profileViewInfoData?.data?.userprofile?.declaration?.dbacheck}>
            <Radio value={true}>Yes</Radio>
            <Radio value={false}>No</Radio>
          </Radio.Group>
        </Form.Item>
        ...
        {conditionalPath ? <div className='candidate-wrapper '>
          ...
          <Col xs={24} md={7} >
            <div className='candidate-wrapper d-flex justify-end  '>
              <Button htmlType='submit' className="candidate-button fw-600 fs-16 cursor-pointer" >
                Send Email
              </Button>
