...
const Rating = () => {
  ...
  const { data, isLoading, isError, isSuccess } = useGetUserOtherInfoRequestQuery({ refetchOnMountOrArgChange: true, userId: id, detail: "ABOUT" });

  let ratingsData: any;
  if (isLoading) {
    ratingsData = <p>Loading...</p>;
  } else if (isSuccess) {
    ratingsData = data;
  } else if (isError) {
    ratingsData = <p>Error...</p>;
  }
