...
const ItHelpTicketSummaryDetails = () => {
  ...
  const { data, isLoading, isSuccess, isError } = useGetHelpDeskDetailsRequestQuery({
  ...
  if (isLoading) {
    helpDeskDetailsData = <p>Loading...</p>;
  } else if (isSuccess) {
    helpDeskDetailsData = data;
  } else if (isError) {
    helpDeskDetailsData = <p>Error...</p>;
  }

  // get  ReportedBy api request  **********************************
  const [getReportedByRequest, { data: reportedByData, isLoading: isLoadingReportedBy, isSuccess: isSuccessReportedBy, isError: isErrorReportedBy }] =
    usePatchAssignToRequestMutation();

  let reportedByFilterData: any;
  if (isLoadingReportedBy) {
    reportedByFilterData = <p>Loading...</p>;
  } else if (isSuccessReportedBy) {
    reportedByFilterData = reportedByData;
  } else if (isErrorReportedBy) {
    reportedByFilterData = <p>Error...</p>;
  }
  ...
  if (isLoadingAssignTo) {
    assignToListData = <p>Loading...</p>;
  } else if (isSuccessAssignTo) {
    assignToListData = assignToData;
  } else if (isErrorAssignTo) {
    assignToListData = <p>Error...</p>;
  }
  ...
  const handleAssignTo = async () => {
    ...
    console.log(assignToData, error);
