...
const StaffManagerInfo = (props: any) => {
  ...
  const handleDeleteModalSubmit = async () => {
    ...
    deleteProfile({ id: staffDetails?._id });
  };
    ...
    },
    {
      label: (
        <div
          ...
          onClick={() =>
            navigate(`/staff-manager/staff-summary/${staffDetails?._id}`, {
              state: staffDetails,
            })
          }
        >
  ...
  return (
    <>
                ...
                <Col xs={24} lg={6} xl={4} className="staff-info">
                  <div className="border-right">
                    ...
                    <div className="staff-manager-heading">
                      <h2>{`${item?.fullName.split(" ")[0]?.charAt(0).toUpperCase()}${item?.fullName.split(" ")[0]?.slice(1).toLowerCase()} ${item?.fullName
                        .split(" ")[1]
                        ?.charAt(0)
                        .toUpperCase()}${item?.fullName.split(" ")[1]?.slice(1).toLowerCase()}`}</h2>
      ...
      />
      <SendResendModal isModalOpen={isModalOpen} setIsModalOpen={setIsModalOpen} data={staffDetails} />
      <AllocateStaffModal staffId={staffId} allocateStaff={allocateStaff} setAllocateStaff={setAllocateStaff} />
      <SendEmailModal isSendEmailModalOpen={isSendEmailModalOpen} setIsSendEmailModalOpen={setIsSendEmailModalOpen} />
      {/* <ViewProfile staffDetails={staffDetails} IsProfileModal={IsProfileModal} setIsProfileModal={setIsProfileModal} selectedTableData={profileViewData} selectProfileData={selectProfileData} /> */}
    </>
  );
};
