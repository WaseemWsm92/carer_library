...
const CompletedConfirmModal = (props: any) => {
  const { isCompletedConfirmModal, setIsCompletedConfirmModal, shiftIdData } = props;
  ...
  const handleCompletedShift = async () => {
    ...
    const {data: completed} = await createConfirm({ staffShiftId: shiftIdData._id, shiftStatus: 'SIGNEDOFF' });
    const {data} = await addRating({ carerId: shiftIdData?.carer._id, shiftId: shiftIdData._id, rating: ratingValue.rating, feedback: ratingValue.review });
  }
      ...
      <Modal title="Shift Information" open={isCompletedConfirmModal} onCancel={() => setIsCompletedConfirmModal(false)} footer={false} className="confirm-modal-wrapper" width={850} centered>
        <div className="confirm-modal-content">
          ...
          <div className="shift-btn-wrapper d-flex align-center" style={{paddingTop: "30px"}}>
            <button type="button" className="cancel-btn fs-16 fw-600 white-color m-0 cursor-pointer">
              Cancel
            </button>
