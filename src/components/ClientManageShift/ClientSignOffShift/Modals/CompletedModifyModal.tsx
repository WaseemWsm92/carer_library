...
const CompletedConfirmModal = (props: any) => {
  const { isCompletedModifyModal, setIsCompletedModifyModal, shiftIdData } = props;
  ...
  const handleCompletedConfirmShift = async () => {
    const { data: completed } = await createConfirm({
  ...
  return (
    <>
      <Modal title="Shift Information" open={isCompletedModifyModal} onCancel={() => setIsCompletedModifyModal(false)} footer={false} className="confirm-modal-wrapper" width={850} centered>
      ...
      <ConfirmAndSignOffModal open={isConfirmModalOpen} onCancel={() => setIsConfirmModalOpen(false)} ConfirmButton={handleCompletedConfirmShift} heading={'Are you sure you want to sign off this shift’s (Modified) Check in and Check out time'} CancelButton={() => setIsConfirmModalOpen(false)} />
    </>
  );
};
