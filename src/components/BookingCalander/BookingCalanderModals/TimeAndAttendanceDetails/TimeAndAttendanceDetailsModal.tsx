...
function TimeAndAttendanceDetailsModal(props: any) {
  ...
  const shiftInfoMock = [
    { heading: "Shift Duration", detail: dayjs(eventDetails?.shiftDate).format('DD/MM/YYYY') },
    { heading: "Start Time", detail: dayjs(eventDetails?.shiftStartTime).format('HH:MM a') },
    { heading: "End Time", detail: dayjs(eventDetails?.shiftEndTime).format('HH:MM a') },
  ];
  return (
    <Modal
      centered
      ...
      <Row>
        <Col xs={12} sm={12} md={12} style={{ margin: "2rem 0 3rem 0" }}>
          ...
          <div className="detail-wrapper d-flex">
            ...
            <span className="fs-12 fw-500" style={{ marginLeft: "10px" }}>{dayjs(eventDetails?.shiftDate).format('DD/MM/YYYY')}</span>
          </div>
        </Col>
