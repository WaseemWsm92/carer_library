...
function ShiftInfo(props: any) {
  ...
  const shiftInfoMock = [
    { heading: "Posted At", detail: dayjs(shiftDetails?.shiftDate).format('DD/MM/YYYY') },
    { heading: "Requested by", detail: shiftDetails?.requestedBy },
    { heading: "Entered by", detail: shiftDetails?.requestedBy },
    { heading: "Start Time", detail: dayjs(shiftDetails?.shiftStartTime).format('hh:mm a') },
    { heading: "EndTime", detail: dayjs(shiftDetails?.shiftEndTime).format('hh:mm a') },
  ...
  const handleCancelShift = async (reason: any) => {
    ...
    if(!error) setIsCancellationModalOpen(false)
  }
              ...
              {shiftInfoMock.map((data) => {
                return (
                  <Fragment key={uuidv4()}>
                    ...
                    <Col xs={12} md={10} className='text-end'>{data.heading === 'Staff List' ?
                      data?.detail?.map((user: any) => (`${user?.staff?.firstName} ${user?.staff?.lastName}, `)) : data.detail}</Col>
                  </Fragment>
                );
              })}
            </Row>
        ...
        <Row>
          {eventStatus !== "COMPLETED" && (
            <Col xs={24} className="btn-wrapper d-flex">
              {(eventStatus === "UNPUBLISHED" || eventStatus === "PUBLISHED") ? <Button type="primary" onClick={() => navigate(`/booking-calendar/staff-availability`)} className="alocate-shift-btn fs-14 fw-600">Allocate Shift</Button> :
                <Button
                  type="primary"
