...
function TimeSheetDetailsModal(props: any) {
  ...
  console.log(eventDetailsData?.event?._def?.extendedProps, 'eventDetails');
              ...
              <Row gutter={[10, 1]}>
                <Col xs={24} md={12}>
                  <Form.Item label={<span className="label-color">Shift Start Time</span>}>
                    <Input type="text" style={{ border: "1px solid #a0a3bd", borderRadius: "3px" }} size="large" defaultValue={dayjs(eventDetails?.shiftStartTime).format(('DD/MM/YYYY, HH:MM'))} disabled />
                  </Form.Item>
                </Col>
                <Col xs={24} md={12}>
                  <Form.Item label={<span className="label-color">Shift End Time</span>}>
                    <Input type="text" style={{ border: "1px solid #a0a3bd", borderRadius: "3px" }} size="large" defaultValue={dayjs(eventDetails?.shiftEndTime).format(('DD/MM/YYYY, HH:MM'))} disabled />
                  </Form.Item>
                </Col>
                <Col xs={24} md={12}>
                  <Form.Item label={<span className="label-color">Shift Completed Time</span>}>
                    <Input type="text" style={{ border: "1px solid #a0a3bd", borderRadius: "3px" }} size="large" defaultValue={eventDetails?.timeTrack.length ? dayjs(eventDetails?.timeTrack[eventDetails?.timeTrack.length - 1]?.checkOut).format(('DD/MM/YYYY, HH:MM')) : ""} disabled />
                  </Form.Item>
                </Col>
                <Col xs={24} md={12}>
                  <Form.Item label={<span className="label-color">Shift Signed-off Time</span>}>
                    <Input type="text" style={{ border: "1px solid #a0a3bd", borderRadius: "3px" }} size="large" defaultValue={eventDetails?.signedOffDate ? dayjs(eventDetails?.signedOffDate).format(('DD/MM/YYYY, HH:MM')) : ""} disabled />
                  </Form.Item>
                </Col>
              </Row>
            </Form>
            ...
            <Form layout="vertical">
              <Row gutter={[10, 1]}>
                <Col xs={24} md={12}>
                  <Form.Item label={<span className="label-color">Extra Hours</span>} style={{ marginBottom: 0 }}>
                    <Input type="text" style={{ border: "1px solid #a0a3bd", borderRadius: "3px" }} size="large" defaultValue={eventDetails?.extraHours ? eventDetails?.extraHours : 0} disabled />
                  </Form.Item>
              ...
              <Row>
                <Col xs={24} md={24} style={{ display: "flex", justifyContent: "space-between" }}>
                  ...
                  <span className="fs-14 ">{eventDetails?.signedOffBy?.firstName} {eventDetails?.signedOffBy?.lastName}, {dayjs(eventDetails?.signedOffDate).format(('DD/MM/YYYY, HH:MM'))}</span>
                </Col>
                <Col xs={24} md={24} style={{ display: "flex", justifyContent: "space-between" }}>
                  <span className="fs-14 fw-600">Modified by:</span>
                  <span className="fs-14 ">{eventDetails?.modifiedBy?.firstName} {eventDetails?.modifiedBy?.lastName}, {dayjs(eventDetails?.modifiedDate).format(('DD/MM/YYYY, HH:MM'))}</span>
                </Col>
                <Col xs={24} md={24}>
