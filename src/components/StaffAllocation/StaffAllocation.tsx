...
const StaffAllocation = () => {
  ...
  const paramsObj: any = {};
  if (serarchTerm) paramsObj["search"] = serarchTerm;
  ...
  const downloadedAsXls : any = useDownloadStaffRecordQuery({type:'xls',id:defaultId})
  const downloadedAsCsv : any = useDownloadStaffRecordQuery({type:'csv',id:defaultId})

  // console.log(downloadedAsCsv, 'cvs')
...
//   fetch('https://gateway.dev.carelibrary.developersorcalo.com/staff/download?downloadType=xls&clientId=643631c0f57c54ac09cf12fc', {
//   headers: {Authentication: `Bearer ${token?.token}`,'Content-Type': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', 'Access-Control-Allow-Credentials': 'true', 'Content-Disposition': 'attachment; filename="data.xlsx"'}
//   })
//    .then(resp => resp.blob()).then((blob) => console.log(blob))
      ...
      <div className="staff-allocation-main-wrapper">
        ...
        <div className="table-fnc-head-wrapper">
          <StaffAllocationTable
            ...
            xlsData={downloadedAsXls}
            csvData={downloadedAsCsv}
            setSelectedRowKeys={setSelectedRowKeys}
            selectedRowKeys={selectedRowKeys}
            setFilterValues={setFilterValues}
            loading={staffLoad}
