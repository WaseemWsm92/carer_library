...
const ManageNotificationTable = ({ searchValue }: any) => {
  ...
  const { data: notificationsList, isLoading } = useGetNoticationsListQuery(searchValue);
  ...
  return (
    <div className="main-notifications-wrapper">
      <div className="carer-main">
        <Table loading={isLoading} className="wrapper-table" columns={columns} dataSource={notificationsList?.data} scroll={{ x: "max-content" }} bordered={false} pagination={false} />
      </div>
