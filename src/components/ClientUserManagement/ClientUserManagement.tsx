...
const ClientUserManagement = () => {
  ...
  const { data: getUsers, isLoading } = useGetClientUserQuery({ department: userDepartmentFilter, search: userSearch });
  ...
  const items: MenuProps['items'] | undefined = [
    {
      label: (
        <div className="d-flex align-center" style={{ gap: "18px", padding: "12px" }} onClick={() => {setAddUserModalOpen(true); setActionType('edit')}}>
  ...
  const columns: ColumnsType<any> = [
    ...
    },
    {
      ...
      key: 'createdAt',
      render: (text: any) =>
        <span className='fs-14 fw-400 m-0 line-height-22 title-color'>{dayjs(text).format('DD-MM-YYYY')}</span>,
    },
    {
      title: 'Actions',
      key: 'actions',
  ...
  return (
    <div className='client-user-management-wrapper'>
      <div className='client-user-wrap bg-white'>
        <Button type='primary' onClick={() => {setAddUserModalOpen(true); setActionType('add'); setUserData({})}}>Add Admin User</Button>
        <div className='client-user-select'>
          <SelectWrapper
            ...
            label="Department"
            required={false}
            onChange={(e: any) => setUserDepartmentFilter(e)}
            placeHolder="All"
          />
        </div>
      </div>
      ...
      <div className='client-user-manangement-table'>
        <Table columns={columns} dataSource={getUsers?.data?.result} loading={isLoading} pagination={false} className="client-user-manangement-table-content" scroll={{ x: "max-content" }} />
      </div>
      <DeleteModal
      ...
      />
      <AddAdminUserModal setAddUserModalOpen={setAddUserModalOpen} addUserModalOpen={addUserModalOpen} userData={userData} actionType={actionType} setActionType={setActionType} />
