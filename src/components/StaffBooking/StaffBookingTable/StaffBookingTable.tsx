...
const StaffBookingTable = (props: any) => {
  ...
  const items: any = [
    ...
    {
      label: (
        <Space onClick={() => navigate(`/staff-booking/completed-shift/${staffBookingId}`)}>
  ...
  return (
    <div className="">
      <Table columns={columns} dataSource={data} pagination={false} className="staff-booking-table" scroll={{ x: "max-content" }} />
    </div>
  );
};
