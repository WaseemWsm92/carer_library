    ...
    <>
      <div className="client-recent-reviews-wrappper bg-white">
        ...
        <div className="recent-reviews-content">
          {recentReviewsData?.data?.slice(0, 5).map((item: any) => (
            <div className="recent-reviews-items d-flex align-center" key={item?._id}>
              {/* <Avatar src={`https://rnd-s3-public-dev-001.s3.eu-west-2.amazonaws.com/${item?.userInfo?.profilePhoto}`}>{item?.userInfo?.firstName}</Avatar> */}
              {/* <img src={item?.userInfo?.profilePhoto?`https://rnd-s3-public-dev-001.s3.eu-west-2.amazonaws.com/${item?.userInfo?.profilePhoto}`: `https://ui-avatars.com/api/?name=${item?.userInfo?.firstName}&border-radius=10`} alt={"img"} /> */}
              <img width={30} src={`https://ui-avatars.com/api/?name=${item?.userInfo?.firstName}+${item?.userInfo?.lastName}`} alt={"img"} />
