...
const LogsTable = () => {
  ...
  const { data, isLoading }:any = useGetAuditLogsRequestQuery({page:1,limit:10, query });
  ...
  return (
    <div className="audit-logs-wrapper">
      ...
      <div className="searchbox">
        <Table loading={isLoading} columns={ColumnsData} dataSource={data?.result} pagination={false} scroll={{ x: "max-content" }} />
      </div>{" "}
    </div>
  );
};
