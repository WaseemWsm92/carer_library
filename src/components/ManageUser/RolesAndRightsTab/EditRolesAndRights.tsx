...
import { usePostUserRightsMutation, usePutUserRightsMutation } from "../../../store/Slices/ManageUser";
...
const EditRolesAndRights = ({
  ...
  const handleFinish = (values: any) => {
    ...
    if (type === "add") {
      ...
      putUserRights({ payload: { permissions: checkedValues }, id: tableRowRecord._id });
  ...
  const handleChange = ({ ele, typeName, types }: any) => {
    if (typeName === "Parent") {
      ...
      let permissionIndex = checkedValues?.findIndex((item: any) => item?.name === ele);
      if (permissionIndex !== -1) {
        let updatedPermissions: any[] = [...checkedValues]; // create a new array
        let typesIndex = updatedPermissions[permissionIndex].types?.indexOf(types);
        if (typesIndex !== -1) {
          updatedPermissions[permissionIndex].types = updatedPermissions[permissionIndex].types.filter((type: any) => type !== types); // remove types value from array
        } else {
          updatedPermissions[permissionIndex].types = [...updatedPermissions[permissionIndex].types, types]; // add types value to array
        }
      ...
      <Row gutter={[30, 30]}>
        ...
        >
          ...
          >
            ...
            <Input placeholder="Type here" disabled={type === "add" ? false : true} />
          </Form.Item>
        </Col>
