...
const RolesAndRights = (props: any) => {
  ...
  const handleAddRole = () => { setIsEditRole(true); setType("add") };
  const handleEditModal = () => setIsEditRole(true);
  const { } = useUserRightsResetPasswordQuery(tableRowRecord?._id, { skip });

  const paramsObj: any = {};
  if (searchName) paramsObj["search"] = searchName;
  const query = "&" + new URLSearchParams(paramsObj).toString();
  const { isError, isSuccess, isLoading, data } = useGetUserRightsQuery({
    roleId: id,
    query,
  });
            ...
            >
              <Space key={uuidv4()} onClick={() => setTableRowRecord(record)}>
                <div className="border-color cursor-pointer">
                  <img src={ActionIcon} alt="" onClick={() => setTableRowRecord(record)} />
                </div>
              </Space>
            </Dropdown>
          )}
