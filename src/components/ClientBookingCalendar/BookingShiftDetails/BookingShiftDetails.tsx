...
const BookingShiftDetails = () => {
  ...
  const { data, isLoading: getShiftLoading, isSuccess } = useGetShiftDetailsDataQuery({ userId: id, query });
    ...
    <>
      <div className='clients-booking-shift-details'>
        ...
        <div className='shift-booking-table'>
          <Table columns={columns} dataSource={getBookingShiftDetails?.data?.shifts} pagination={false} loading={getShiftLoading} className="booking-shift-table-content" scroll={{ x: "max-content" }} />
        </div>
      </div>
