  ...
  endpoints: (builder) => ({
    getShiftLists: builder.query({
      query: ({ careHomeId, startTime, endTime }) => ({
        url: `shifts?limit=100${careHomeId && `&careHomeId=${careHomeId}`}${startTime && `&startTime=${startTime}`}${endTime && `&endTime=${endTime}`}&shiftStatus=UNPUBLISHED&shiftStatus=PUBLISHED&shiftStatus=BOOKED&shiftStatus=COMPLETED&shiftStatus=INPROCESS`,
        method: "GET",
      }),
      providesTags: ["shifts"],
    }),
    ...
    getStaffList: builder.query({
      query: () => ({
        url: "/manage-user?roleName=carer",
        method: "GET",
      }),
    }),
